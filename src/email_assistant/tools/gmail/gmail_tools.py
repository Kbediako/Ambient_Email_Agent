...
def fetch_emails_tool(email_address: str, minutes_since: int = 30) -> str:
	...
	for i, email_item in enumerate(emails, 1):
		if email_item.get("user_respond", False):
			result += f"{i}. You already responded to this email (Thread ID: {email_item['thread_id']})\n\n"
			continue
			
		result += f"{i}. From: {email_item['from_email']}\n"
		result += f"   To: {email_item['to_email']}\n"
		result += f"   Subject: {email_item['subject']}\n"
		result += f"   Time: {email_item['send_time']}\n"
		result += f"   ID: {email_item['id']}\n"
		result += f"   Thread ID: {email_item['thread_id']}\n"
		result += f"   Content: {email_item['page_content'][:200]}...\n\n"
...
def mark_as_spam(
	...
	"""Move a Gmail message to Spam by applying the SPAM label and removing INBOX.
